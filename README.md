...
The main entry point of EmSI is `SwAV.py` (For serial running, please use `Serial_swav.py`). For slurm system, please see the heading 
...
### P1-P4
#### EmSI running instructions

The main entry point is  `main.py`.   Scripts:.
* Script for Phase 1 type inference is  [`EmSI/P1.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P1.sh)), the baseline script is [`EmSI/runBaselineP1.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/runBaselineP1.sh).
* Script for Phase 2 conceptual attribute inference and Phase 3 hierarchy inference is  [`EmSI/P23.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P23.sh), 
the baseline script is [`EmSI/runBaselineP23.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/runBaselineP23.sh).
* Script for Phase 4 conceptual attribute relationship search is [`EmSI/P4.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P4.sh).
Hyperparameters
...
This repository also includes the implementation for the supplementary experiments reported in the revision/response.

### Gaussian Noise Robustness Test

The Gaussian noise robustness experiment is implemented in:

- [`EmSI/GaussianNoise.py`](https://github.com/PierreWoL/SILM/blob/main/EmSI/GaussianNoise.py)

This script adds Gaussian noise to the weighted table embeddings used by EmSI. It also includes a sanity check based on pairwise Euclidean distances between table embeddings, which is used to calibrate the perturbation strength.

The generated noisy embedding files can be directly used as input to Phase 1 of EmSI. To run Phase 1 with the generated embeddings, use:

- [`EmSI/P1.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P1.sh)

### Controlled Experiments with Varying Dataset Properties

We also provide scripts for the controlled experiments that vary dataset properties such as schema size, attribute overlap, and schema/header noise levels.

The final generated datasets are not included in the repository because they are dynamically generated by the following scripts.

#### Schema Size

The schema-size controlled datasets are generated by:

- [`SchemaSize.py`](https://github.com/PierreWoL/SILM/blob/main/SchemaSize.py)

This script creates dataset variants by sampling different proportions of tables from the original dataset.

#### Attribute Overlap

The attribute-overlap controlled datasets are generated by:

- [`AttributeOverlap.py`](https://github.com/PierreWoL/SILM/blob/main/AttributeOverlap.py)

This script creates dataset variants according to different levels of attribute overlap between tables and their corresponding ground-truth types.

#### Schema/Header Noise Levels

The schema/header noise controlled datasets are generated by:

- [`SchemaPerturbation.py`](https://github.com/PierreWoL/SILM/blob/main/SchemaPerturbation.py)

This script perturbs column names to simulate different levels of schema/header noise.

### Running the Generated Controlled Datasets

After generating a controlled dataset, place the generated dataset folder under the `datasets/` directory.

Then update the dataset parameter in the corresponding running scripts to point to the generated dataset folder:

- [`EmSI/P1.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P1.sh)
- [`EmSI/P23.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P23.sh)
- [`EmSI/P4.sh`](https://github.com/PierreWoL/SILM/blob/main/EmSI/P4.sh)

For example, replace the dataset argument with the name of the generated dataset folder before running the corresponding phase.