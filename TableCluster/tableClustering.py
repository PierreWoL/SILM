...
def silm_clustering(hp: Namespace):
    ...
    files =[fn for fn in os.listdir(datafile_path) if '.pkl' in fn and hp.embed in fn and "subCol" in fn]
    if hp.subjectCol:
    ...
    for file in files[1:]:
        ...
        Z = np.array(Z)
        try:
            clustering_method = ["Agglomerative","KMeans","DBSCAN", "BIRCH","GMM","OPTICS"] #, "BIRCH"
            methods_metrics = {}
