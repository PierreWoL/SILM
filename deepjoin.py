...
import torch
import pickle
from Deepjoin.train import construct_train_dataset, train_model2
...
if __name__ == "__main__":
    ...
    if os.path.isfile(os.path.join(f"datasets/{dataset}/", f'trainDeepJoinData{dataset}.pickle')):
      ...
      print("Read successfully")
        ...
        with open(os.path.join( f"datasets/{dataset}/", f'trainDeepJoinData{dataset}.pickle'), 'wb') as f:
            pickle.dump(train_dataset, f)
        print("Succeeded in  building and saving training dataset...")
    train_model2(model_name=model_name, train_dataset=train_dataset, dev_samples=None, model_save_path=f"model/WDC/Deepjoin{dataset}.pt",
