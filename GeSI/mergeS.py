...
def merge_and_evaluate(dataset, target_path, json_file, test_threshold=0.6, test_Whole = False):
    ...
    print(json_file)
    for line in data:
        ### TODO this needs to be deleted later
...
json_types = [ 'results', 'results_constraintSTL','results_constraintFET', 'results_constraintFull', 'results_constraintABS']
dataset_p = "GDS"
model = "deepseek"  # GPT3
shots = ['3']  # '0', '1', '3'
thres = [0.5]
for shot in shots:
    for json_type in json_types:
        for thre in thres:
            target_p = f"Result/{dataset_p}/Prompt0/{shot}/{model}/{json_type}/{thre}/selected/"  # /refine
            mkdir(target_p)
            json_file_p = os.path.join(f"Result/{dataset_p}/Prompt0/{shot}/{model}/", f"{json_type}.jsonl")
